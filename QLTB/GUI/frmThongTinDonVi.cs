...
namespace QLTB.GUI
{
    public partial class frmThongTinDonVi : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void loadForm()
        {
            ...
            //
            cbbKhoiTruong.DataSource = dshandler.GetKhoiTruong().Select(p => new { key = p.KhoiTruongId, value = p.Ten }).ToList();
            cbbLoaiTruong.DataSource = dshandler.GetLoaiHinhTruong().Select(p => new { key = p.LoaiHinhTruongId, value = p.Ten }).ToList();
            cbbLoaiTruongCB.DataSource = dshandler.GetLoaiTruongChuyenBiet().Select(p => new { key = p.LoaiTruongChuyenBietId, value = p.Ten }).ToList();
            cbbCapTruong.DataSource = dshandler.GetCapTruong().Select(p => new { key = p.CapTruongId, value = p.Ten }).ToList();
            cbbDacBietVM.DataSource = dshandler.GetVungMien().Select(p => new { key = p.VungMienId, value = p.Ten }).ToList();
            cbbHangTruong.DataSource = dshandler.GetHangTruong().Select(p => new { key = p.HangTruongId, value = p.Ten }).ToList();
            cbbHinhThucDT.DataSource = dshandler.GetHinhThucDaoTao().Select(p => new { key = p.HinhThucDaoTaoId, value = p.Ten }).ToList();
